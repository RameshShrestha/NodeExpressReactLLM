...
import {getDetailFromURL} from './getDetailFromURL.js'
const red = '\x1b[31m';
const green = '\x1b[32m';
const blue = '\x1b[34m';
const reset = '\x1b[0m';
...
router.route("/").post(async (req, res) => {
  ...
  let imageBuffer = [];
  let { modelName, prompt, systemPrompt, chatHistory, stream ,addedURL} = req.body;
  if (stream === 'false') {
    stream = false;
  } else if (stream == 'true') {
    stream = true;
  ...
  if(addedURL){
    console.log("URL is provided : ", addedURL);
    fetchedURLContent = await getDetailFromURL(addedURL);
    console.log("Total Length : ", fetchedURLContent.length);
    if(fetchedURLContent.length> 10000){
      fetchedURLContent = fetchedURLContent.substring(0,10000);
    }
  }else{
  ...
  if (req.files) {
    ...
    for (let file of req.files) {
      if (file.mimetype.indexOf("image") > -1) {
        ...
        console.log("test base 64", base64String.indexOf('data:image'));
        const cleanBase64 = base64String.replace(/^data:image\/\w+;base64,/, "");

        imageBuffer.push(cleanBase64);
 ...
 if(fetchedURLContent){
  systemPrompt = systemPrompt + `\n\n The user has provided the URL to fetch the details and the operation is performed and got the result  \n\n ${fetchedURLContent}
   You provide your response based on the content which has been fetched no need to make other tools again.
  ...
  if (modelName && prompt && systemPrompt) {
    ...
    if (RAG_Content) {
      ...
      response = await normalchat(modelName, systemPrompt, chatHistory, prompt, stream, imageBuffer);
      console.log(`${red} RAG realted search so no tools are used ${reset}`);
    } else {
      response = await chatWithTools(modelName, systemPrompt, chatHistory, prompt, stream, imageBuffer);
    }
    ...
    if (stream) {
      ...
      for await (const chunk of response) {
        if (chunk?.message?.tool_calls) {
          ...
          if (chunk.message.tool_calls[0].function.name === "getDetailsFromWeb") {
            ...
            const responseWithTools = await normalchat(modelName, systemPrompt, newChatHistory, prompt, stream, imageBuffer);
            let collectedResponse = "";
      ...
      if (responseContent.name === "getDetailsFromWeb" && responseContent.arguments && responseContent.arguments.query_topic) {
        ...
        const responseWithTools = await normalchat(modelName, newSystemPrompt, newChatHistory, prompt, stream, imageBuffer);
